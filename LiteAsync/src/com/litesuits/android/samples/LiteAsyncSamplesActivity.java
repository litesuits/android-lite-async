...
public class LiteAsyncSamplesActivity extends BaseActivity {
	...
	/**
	 * 
	 * 0<item>AsyncTask Execute</item>
	 * 1<item>AsyncTask Exe Runnable</item>
	 * 2<item>Simple Task</item>
	 * 3<item>Safe Task</item>
	 * 4<item>Task Cancel</item>
	 * 5<item>Ordered Task Executor</item>
	 * 6<item>CyclicBarrier Task Executor</item>
	 * 7<item>Delayed Task</item>
	 * 8<item>Timer Task</item>
	...
	private void testCachedAsyncTask() {
		...
		new SimpleCachedTask<User>(LiteAsyncSamplesActivity.this, "getUserInfo", 10, TimeUnit.SECONDS) {
			@Override
	...
	private void testSimpleTask() {
		...
		SimpleSafeTask<String> sst = new SimpleSafeTask<String>() {
			...
			protected void onPostExecuteSafely(String result, Exception e) throws Exception {
				Log.i("SimpleSafeTask onPostExecuteSafely " + result + " , thread id  : " + Thread.currentThread().getId());
			}
		};
		sst.execute();
		...
		SimpleSafeTask<String> sse = new SimpleSafeTask<String>() {
			...
			@Override
			protected void onProgressUpdateSafely(Object... values) throws Exception {
	...
	private void testSafeTask() {
		...
		SafeTask<Integer, Integer, String> se = new SafeTask<Integer, Integer, String>() {
			...
			@Override
			protected void onPostExecuteSafely(String result, Exception e) throws Exception {
				Log.i("SafeTask error onPostExecuteSafely :" + result + ",  thread id  : " + Thread.currentThread().getId());
				Log.i("SafeTask error onPostExecuteSafely Exception :" + e + ",  thread id  : " + Thread.currentThread().getId());
				String s = null;
				s.toCharArray();
			}
			...
			protected void onProgressUpdateSafely(Integer... values) {
				Log.i("SafeTask error onProgressUpdateSafely :" + Arrays.toString(values) + ",  thread id  : " + Thread.currentThread().getId());
				String s = null;
				s.toCharArray();
			}
		};
	...
	private void testOrderedTaskExecutor() {
		...
		SimpleTask<?> s3 = getTask(3);
		final long startTime = System.currentTimeMillis();
		SimpleTask<?> s4 = getTask(4);
		SimpleTask<?> s5 = new SimpleTask<Void>() {
			@Override
			...
			protected void onPostExecute(Void result) {
				Log.i("OrderedTaskExecutor use time: " + (System.currentTimeMillis() - startTime));
			}
		};
		TaskExecutor.newOrderedExecutor().put(s3).put(s1).put(s2).put(s4).put(s5).start();
		s2.cancel(true);
	...
	private void testCyclicBarrierExecutor() {
		SimpleTask<?> t1 = getTask(1);
		SimpleTask<?> t2 = getTask(2);
		SimpleTask<?> t3 = getTask(3);
		SimpleTask<?> t4 = getTask(4);
		final long startTime = System.currentTimeMillis();
		SimpleTask<String> t5 = new SimpleTask<String>() {
			@Override
			...
			protected void onPostExecute(String result) {
				Log.i("CyclicBarrierExecutor use time: " + (System.currentTimeMillis() - startTime) + " , info: " + result);
			}
		};
		TaskExecutor.newCyclicBarrierExecutor().put(t1).put(t4).put(t2).put(t3).start(t5);
		t2.cancel(true);
		SystemClock.sleep(1500);
		t1.cancel(true);
	}
	...
	public static class User extends BaseResponse {
		...
		public static class UserInfo implements Serializable {
			...
			public String toString() {
				return "UserInfo [name=" + name + ", age=" + age + ", girl_friends=" + girl_friends + "]";
			}
	...
	public static abstract class BaseResponse implements Serializable {
		...
		@Override
		public String toString() {
			return "BaseResponse [api=" + api + ", v=" + v + ", result=" + result + "]";
		}
	}
